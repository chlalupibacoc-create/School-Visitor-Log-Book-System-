...
def add_visitor(data):
    ...
    while True:
        ...
        if not valid_word(purpose):
            print("❌ Purpose must contain letters only.")
            continue
        break
    ...
    data.append(visitor)
    save(data)
    print(f"✅ {name} logged in at {time_in}")
...
def mark_exit(data):
    ...
    for v in data:
        if v["name"].lower() == name.lower() and v["time_out"] is None:
            ...
            save(data)
            print(f"✅ {name} logged out at {v['time_out']}")
            return
    print("❌ Visitor not found or already exited.")
